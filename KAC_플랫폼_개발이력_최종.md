...
| 새로고침 시 잠금화면 | silent=true 플래그로 overlay 미표시, 현황판 유지 |
...
| 강의실 현황판 강의실 열 | "마지막 활동" → "강의실"로 변경, roomDetailName 2줄 가운데 표시 |
| Reset 시 강의실 초기화 | settings.roomDetailName Reset 시 함께 초기화 |
| 외출/외박 복귀여부 | 테이블에 복귀여부(✅/⏳) + 복귀호출 컬럼 추가 |
| 전체 복귀호출 버튼 | 미복귀자 전체 일괄 Firebase 신호 발송, 하단 우측 배치 |
| 주야간 버튼 제거 | 주간 모드 고정 |
| 전체화면(사이드바 토글) | CSS 클래스 토글 방식, 사이드바 우측에 책갈피 탭(fixed 포지션) |
...
| 파일 | 주요 변경 내용 |
|------|---------------|
| `admin.js` | 공지 리스너, 퀴즈모드, 현황판, Reset 권한, 사이드바 토글, 출결 권한, cancelTakeover 등 |
| `admin.html` | 버튼 정리, 사이드바 탭, 공지 컬러바, 강의실 열 헤더, 로딩 스피너 등 |
| `admin.css` | 카드 높이, 사이드바 슬라이드 애니메이션, LED 색상 등 |
| `admin_coord.html` | 색상 정리, 공지 이력, textarea placeholder 등 |
| `index.html` | 출결 복수강의실 선택, localStorage 초기화, 복귀알림, cleanUrlAndGo 등 |
