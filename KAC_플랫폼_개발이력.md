...
| 접속 로딩 화면 | 데이터 로드 중 흰색 스피너 + 안내 문구 표시, 완료 시 과정명/입장 버튼 표시 |
...
| 파일 | 주요 변경 내용 |
|------|---------------|
| `admin.js` | 공지 리스너, 퀴즈모드, 현황판, Reset 권한, 사이드바 토글, 출결 권한, cancelTakeover, ETA 계산 등 |
| `admin.html` | 버튼 정리, 사이드바 탭, 공지 컬러바, 강의실 열 헤더, 로딩 스피너, ETA 블록 등 |
| `admin.css` | 카드 높이, 사이드바 슬라이드 애니메이션, LED 색상 등 |
| `admin_coord.html` | 색상 정리, 공지 이력, textarea placeholder 등 |
| `index.html` | 출결 복수강의실 선택, localStorage 초기화, 복귀알림, cleanUrlAndGo, resolveRoomFromCode, 접속 로딩 스피너 등 |
