...
loadAttendanceView: function() {
    ...
    toggleAttendanceMode: function(mode) {
        ...
        if(mode === 'official') {
            ...
            // [J19] OTP 출결 진입 시 자동으로 전체화면 표시 — 강의실 뒷자리 교육생도 번호가 보이도록
            setTimeout(() => { try { ui.openOtpFullscreen(); } catch(e){} }, 150);
        }
    },
    ...
    openOtpFullscreen: function() {
        ...
        ov.id = 'otpFsOverlay';
        ov.style.cssText = 'position:fixed; inset:0; z-index:2147483200; background:linear-gradient(160deg,#0f2057 0%,#1a3a8a 70%,#1e40af 130%); display:flex; flex-direction:column; align-items:center; justify-content:center; gap:2vh; cursor:default;';
        ov.innerHTML =
            ...
            + '<button id="otpFsClose" style="position:fixed; top:22px; right:24px; padding:13px 26px; border:2.5px solid rgba(255,255,255,.85); border-radius:14px; background:rgba(255,255,255,.14); color:#fff; font-size:17px; font-weight:900; cursor:pointer; display:inline-flex; align-items:center; gap:9px;"><i class="fa-solid fa-xmark"></i> 닫기 (ESC)</button>';
        ...
        // 모니터 전체화면 진입 (브라우저 정책상 실패해도 오버레이는 전체 화면을 덮음)
        try { if (ov.requestFullscreen) ov.requestFullscreen().catch(()=>{}); } catch(e){}
        const close = () => ui.closeOtpFullscreen();
        document.getElementById('otpFsClose').onclick = (e)=>{ e.stopPropagation(); close(); };
        this._otpFsKey = (e)=>{ if(e.key==='Escape') close(); };
        document.addEventListener('keydown', this._otpFsKey);
        this._otpFsFsChange = ()=>{ if(!document.fullscreenElement && document.getElementById('otpFsOverlay')) close(); };
    ...
    /* [J19] OTP 안내자료 이미지만 전체화면 보기 (클릭/ESC로 닫기) */
    openOtpGuideFullscreen: function() {
        const src = (document.getElementById('otpGuideImg')||{}).src;
        if (!src) return;
        const exist = document.getElementById('otpGuideFs'); if (exist) exist.remove();
        const ov = document.createElement('div');
        ov.id = 'otpGuideFs';
        ov.style.cssText = 'position:fixed; inset:0; z-index:2147483300; background:rgba(0,0,0,.93); display:flex; align-items:center; justify-content:center; padding:16px; cursor:zoom-out;';
        const img = document.createElement('img');
        img.src = src;
        img.style.cssText = 'max-width:100%; max-height:100%; object-fit:contain; border-radius:10px; box-shadow:0 10px 40px rgba(0,0,0,.5);';
        const close = document.createElement('button');
        close.innerHTML = '<i class="fa-solid fa-xmark"></i> 닫기 (ESC)';
        ...
        const remove = () => { const e=document.getElementById('otpGuideFs'); if(e) e.remove(); document.removeEventListener('keydown', onKey); try{ if(document.fullscreenElement) document.exitFullscreen().catch(()=>{}); }catch(err){} };
...
setMode: function(mode) {
        ...
        if (mode === 'home') {
            ...
            // [J18→J20] 홈 화면이 뜨면 검색창에 실제 포커스 → 마우스 클릭 없이 키보드 입력이 바로 들어감(한글 첫 글자 포함).
            //  homeSearchFocus가 '포커스만으로는 메뉴를 숨기지 않도록' 바뀌어(J20) 자동 포커스해도 아래 배너·통계가 유지됨.
            //  진짜 커서가 깜빡이므로 CSS 가짜 커서는 :focus-within 규칙에 의해 자동으로 숨겨짐.
            //  PC(포인터 있는 기기)에서만 적용 — 모바일은 키보드 자동 팝업 방지 위해 제외.
            try {
                var _isFinePointer = !window.matchMedia || window.matchMedia('(pointer:fine)').matches;
                if (_isFinePointer) {
                    var _tryFocus = function(){
                        var el = document.getElementById('homeSearchInput');
                        if (!el || state.currentMode !== 'home') return;
                        var ae = document.activeElement;
                        // 사용자가 그새 다른 입력창을 쓰고 있으면 방해하지 않음
                        if (ae && ae !== document.body && (ae.tagName==='INPUT'||ae.tagName==='TEXTAREA'||ae.isContentEditable)) return;
                        try { el.focus({ preventScroll: true }); } catch(e){ el.focus(); }
                    };
                    setTimeout(_tryFocus, 260);
                    setTimeout(_tryFocus, 900);   // 초기 로딩 중 다른 스크립트가 포커스를 뺏는 경우 재시도
                }
            } catch(e){}
...
document.addEventListener('keydown', function(e){
    try{
        if (!window.state || state.currentMode !== 'home') return;
        var el = document.getElementById('homeSearchInput');
