...
loadAttendanceView: function() {
    ...
    openOtpFullscreen: function() {
        ...
        if (this._otpAutoFsTimer) { clearTimeout(this._otpAutoFsTimer); this._otpAutoFsTimer = null; }
        // [J23] 안내 이미지 사용 가능 여부 (로드 완료된 경우에만 2단 슬라이드/버튼 노출)
        var _gimg = document.getElementById('otpGuideImg');
        var _guideSrc = (_gimg && _gimg.complete && _gimg.naturalWidth > 0) ? _gimg.src : '';
        ...
        ov.innerHTML =
            ...
            + '</div>'
            // ── 2면: 안내서 ──
            + '<div style="position:relative; width:100%; height:50%; display:flex; align-items:center; justify-content:center; background:linear-gradient(160deg,#0b1020 0%,#111a33 60%,#0f2057 130%); padding:6vh 4vw 4vh;">'
            +   (_guideSrc
                 ? '<div style="max-width:min(1000px,92%); max-height:100%; padding:18px; background:rgba(255,255,255,.06); border:1.5px solid rgba(255,255,255,.18); border-radius:26px; box-shadow:0 18px 60px rgba(0,0,0,.5); display:flex; align-items:center; justify-content:center;">'
                 +   '<img src="'+_guideSrc+'" alt="OTP 출결 안내" style="max-width:100%; max-height:calc(50vh - 12vh - 40px); object-fit:contain; border-radius:16px; border:1px solid rgba(255,255,255,.14); box-shadow:0 8px 28px rgba(0,0,0,.4); background:#fff;">'
                 + '</div>'
        ...
        this._otpFsPane = 0;
        const setPane = (p) => {
            if(!_guideSrc) return;   // 안내자료 없으면 슬라이드 비활성
        ...
        const dEnd=(e)=>{ if(sy==null||!_guideSrc) return; var ey=(e.changedTouches?e.changedTouches[0].clientY:e.clientY); var dy=ey-sy; sy=null; if(Math.abs(dy)<40) return; setPane(dy<0?1:0); };
