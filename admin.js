...
loadAttendanceView: function() {
    ...
    _emWatch: function() {
        ...
        this._emRef.on('value', (s) => {
            ...
            if (!v.on) { const fo = document.getElementById('emFsOv'); if (fo) fo.remove(); }
        });
    },
    openEmFullscreen: function() {
        ...
        if (!ov) {
            ...
            ov.style.cssText = 'position:fixed;inset:0;z-index:2147483000;background:linear-gradient(150deg,#450a0a,#7f1d1d 60%,#991b1b);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;';
            ov.innerHTML = '<div style="font-size:clamp(22px,3vw,38px);font-weight:900;letter-spacing:1px;">🚨 비상 출결 번호 🚨</div>'
                ...
                + '<button onclick="document.getElementById(\'emFsOv\').remove(); if(document.fullscreenElement) document.exitFullscreen().catch(function(){});" style="margin-top:8px;padding:13px 26px;border:none;border-radius:13px;background:rgba(255,255,255,.16);color:#fff;font-weight:900;font-size:16px;cursor:pointer;">✕ 닫기</button>';
...
(function(){
  ...
  function _dragDown(event){
    ...
    // [J29] '길게 눌러 이동' 핸들은 드래그 전용이므로 즉시 드래그 시작 —
    //  기존 0.52초 롱프레스 대기 중 마우스가 움직이면 취소되고, 시작되면 이동분만큼 점프해
    //  '리모컨이 멀리서 뒤늦게 쫓아오는' 체감의 원인. 중앙 원(탭 기능 겸용)만 롱프레스 유지.
    if (_drag.pressEl && _drag.pressEl.classList && _drag.pressEl.classList.contains('hex-drag-handle')) {
      _drag.dragging = true;
    ...
    _drag.timer = setTimeout(function(){
      _drag.dragging = true;
      host.classList.add('is-dragging');
      // [J29] 늦게 시작해도 점프하지 않도록 현재 위치·포인터를 새 기준으로 재설정
      var r2 = host.getBoundingClientRect();
      _drag.originX = r2.left; _drag.originY = r2.top;
      _drag.startX = _drag.lastX != null ? _drag.lastX : _drag.startX;
      _drag.startY = _drag.lastY != null ? _drag.lastY : _drag.startY;
      if(navigator.vibrate) navigator.vibrate(35);
    }, 520);
  }
  function _dragMove(event){
    if(!_drag.timer && !_drag.dragging) return;
    _drag.lastX = event.clientX; _drag.lastY = event.clientY;   // [J29] 롱프레스 시작 시 기준 재설정용
    if(!_drag.dragging){
      // [J29] 중앙 원 롱프레스 대기 — 이동해도 취소하지 않고 계속 대기(9px 취소 규칙 폐지)
      return;
    }
    var host = _drag.host; if(!host) return;
