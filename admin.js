...
(function(){
  ...
  function _dragDown(event){
    ...
    // [J37] 시작 시 '보이는 위치'를 left/top로 고정하고 transform 제거 →
    //  기본 CSS translateY(-50%)가 grab 기준과 어긋나 세로가 튀던 문제 방지(수평에도 안전).
    ...
    _drag.w = rect.width;    // [J32] 크기 1회 캐시 — 이동마다 offsetWidth 읽던 강제 리플로우 제거
    _drag.h = rect.height;   // [J37] 세로 중심 클램프용
  ...
  function _applyDragPos(){
    ...
    var w = _drag.w || 154, h = _drag.h || 154;
    var x = _drag.px - _drag.grabDX;
    var y = _drag.py - _drag.grabDY;
    // [J37] '보이는 중심'이 화면 안에 남도록 클램프(박스 왼쪽 기준 → 중심 기준).
    //  넓은 펼침/투명 박스여도 원이 화면 좌우/상하 끝까지 도달. (기존 innerWidth-80이 원을 일찍 멈추게 하던 원인)
    var MG = 46;   // 화면 가장자리에서 원 중심까지 최소 여백
    x = Math.max(MG - w/2, Math.min(window.innerWidth  - MG - w/2, x));
    y = Math.max(MG - h/2, Math.min(window.innerHeight - MG - h/2, y));
