...
(function(){
  ...
  function _dragDown(event){
    ...
    // '길게 눌러 이동' 핸들은 드래그 전용 — 누르는 즉시 드래그
  ...
  function _applyDragPos(){
    ...
    x = Math.max(-((_drag.w||300)-80), Math.min(window.innerWidth-80, x));
  ...
  function _dragMove(event){
    ...
    if(!_drag.dragging){
      if(Math.hypot(event.clientX-_drag.startX, event.clientY-_drag.startY) > 6){
        _drag.dragging = true;
        host.classList.add('is-dragging');
      } else return;
    }
    event.preventDefault();
    // [J32] rAF 배칭 — 프레임당 1회, 항상 '최신' 커서 좌표만 반영(이벤트 폭주 시 밀림 방지)
