...
const annualPlanMgr = {
    ...
    _applyCurrentWeek: async function(courses) {
        ...
        const runningKeys = new Set();     // 이미 어느 방이든 '운영 중'인 과정(정규화 키) → 중복 배치 금지
        this.ROOMS.forEach(r => {
            ...
            if (st.roomStatus === 'active') runningKeys.add(_normPlanKey(nm, pd));
        });

        // 풀에서 ①잠긴 방과 동일 ②이미 운영 중인 과정 제외 → 중복 슬롯 생성 차단
        const pool = weekCourses.filter(c => {
            if (lockedKeys.has(`${(c.name||'').trim()}|${(c.period||'').trim()}`)) return false;
            if (runningKeys.has(_normPlanKey(c.name, c.period))) return false;
            return true;
        });
