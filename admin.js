...
setMode: function(mode) {
        ...
        if (mode === 'home') {
            ...
            // [J18] 홈 화면이 뜨면 검색창에 실제 포커스 → 마우스 클릭 없이 키보드 입력이 바로 검색창에 들어감.
            //  기존의 파란 커서는 CSS로 그린 '가짜 커서'라 실제 포커스가 아니었음.
            //  PC(포인터 있는 기기)에서만 적용 — 모바일은 자동 포커스 시 키보드가 갑자기 튀어나와 불편하므로 제외.
            try {
                ...
                if (_isFinePointer) {
                    setTimeout(function(){
                        ...
                        // 사용자가 그새 다른 곳을 클릭했으면 방해하지 않음
                        if (el && state.currentMode === 'home' &&
