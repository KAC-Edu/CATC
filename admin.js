...
showFinalSummary: async function() {
        ...
        if(bdEl){
            ...
            const sortedQ = questionStats.slice().sort((a,b)=>{
                const aa = (a.accuracy==null)?999:a.accuracy;
                const bb = (b.accuracy==null)?999:b.accuracy;
                return aa - bb;
            });
            if(sortedQ.length === 0){
                bdEl.innerHTML = `<div style="text-align:center; color:#94a3b8; padding:26px 0;">집계된 응답이 없습니다.</div>`;
            } else {
                bdEl.innerHTML = sortedQ.map(s=>{
                    ...
                    return `<div class="qbd-row">
                        <div class="qbd-head">
                            <span class="qbd-no">Q${s.no}</span>
                            <span class="qbd-title">${escb(s.title)}</span>
                            <span class="qbd-pt">${ptStr}점</span>
                        </div>
                        <div class="qbd-bar"><div class="qbd-fill" style="width:${barW}%; background:${col};"></div></div>
                        <div class="qbd-meta"><span style="color:${col}; font-weight:800;">${accLab}</span><span>정답 ${s.correct} · 오답 ${wrong} / 응답 ${s.answered}명</span></div>
                    </div>`;
                }).join('');
            }
        }
...
/* ══════════════════════════════════════════════════════════════════════
   ...
   · J44 교훈: 학생 수는 '유효 이름'(비어있지 않고 undefined 아님)만 + 중복 이름 1회 — 원시 numChildren 과대집계 방지
   · J45 교훈: 화면마다 제각각 계산하지 않고 이 함수 하나만 공유 — 카운트 불일치 원천 차단
   · 이름 매칭: 공백 제거 + 소문자 정규화 (기사 플랫폼 B5, driver_m DN3에 동일 규칙 인라인 이식)
   반환: { items(신청자, token 포함), nonApp(미신청자 이름 배열),
           counts:{osong,terminal,airport,carApplied,carAssumed,car}, total(신청+미신청) }
   ══════════════════════════════════════════════════════════════════════ */
function kacShuttleMerge(requestsObj, studentsObj){
    const norm = n => String(n||'').replace(/\s+/g,'').toLowerCase();
    const reqs = requestsObj || {};
    const items = Object.keys(reqs).map(k => Object.assign({ token:k }, reqs[k]||{}));
    const counts = { osong:0, terminal:0, airport:0, car:0 };
    const nameSet = {};
    items.forEach(i => { if(i && counts[i.type]!==undefined) counts[i.type]++; if(i && i.name) nameSet[norm(i.name)] = 1; });
    const nonApp = Object.values(studentsObj||{})
        .filter(s => s && s.name && s.name !== 'undefined' && String(s.name).trim())
        .map(s => String(s.name).trim())
        .filter((n, idx, arr) => arr.indexOf(n) === idx)
        .filter(n => !nameSet[norm(n)]);
    counts.carApplied = counts.car;
    counts.carAssumed = nonApp.length;
    counts.car = counts.carApplied + counts.carAssumed;
    return { items: items, nonApp: nonApp, counts: counts, total: items.length + nonApp.length };
}

try { window.ui = ui; window.state = state; window.dataMgr = dataMgr; window.guideMgr = guideMgr; } catch(e) { console.warn('[J46] 전역 노출 실패', e); }

/* [J47] 알림 팝업(customAlertModal) ESC 닫기 — 옵저버 제한 안내 등 알림이 떠 있으면
   ESC 한 번으로 즉시 닫힘(캡처 단계에서 처리해 다른 ESC 핸들러보다 우선, 알림이 없을 때는 무개입).
   배경 클릭·✕ 버튼 닫기는 admin.html I23에서 함께 추가됨. */
document.addEventListener('keydown', function(e){
    if (e.key !== 'Escape') return;
    var m = document.getElementById('customAlertModal');
    if (m && m.style.display === 'flex') {
        e.stopPropagation(); e.preventDefault();
        try { ui.closeAlert(); } catch(err) {}
    }
}, true);
