...
// ===== [학생장 룰렛] 입교한 학생 중에서 무작위 추첨 → 학생장 지정(전 플랫폼 연동) =====
ui.closeLeaderRoulette = function(){
  if(ui._rouletteTimer){ clearTimeout(ui._rouletteTimer); ui._rouletteTimer=null; }
...
ui.openLeaderRoulette = async function(){
  ...
  modal.setAttribute('style','position:fixed;inset:0;z-index:9700;display:flex;align-items:center;justify-content:center;background:rgba(15,23,42,.6);backdrop-filter:blur(4px);padding:20px;');
  modal.onclick=function(e){ if(e.target===modal) ui.closeLeaderRoulette(); };
  modal.innerHTML =
    '<style>'
    + '@keyframes rlPop{0%{transform:scale(.6);opacity:0;}60%{transform:scale(1.12);}100%{transform:scale(1);opacity:1;}}'
    + '@keyframes rlGlow{0%,100%{box-shadow:0 0 0 0 rgba(217,119,6,.45);}50%{box-shadow:0 0 0 16px rgba(217,119,6,0);}}'
    + '.rl-spinning{color:#475569;}'
    + '.rl-winner{color:#b45309;animation:rlPop .5s ease;}'
    + '</style>'
    + '<div style="background:#fff;border-radius:24px;width:min(560px,96vw);max-height:92vh;overflow:auto;box-shadow:0 30px 70px rgba(0,0,0,.4);">'
      + '<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:18px 22px;background:linear-gradient(135deg,#b45309,#f59e0b);color:#fff;">'
        + '<div style="font-size:18px;font-weight:900;"><i class="fa-solid fa-trophy" style="margin-right:8px;"></i>학생장 룰렛</div>'
        + '<button onclick="ui.closeLeaderRoulette()" style="background:rgba(255,255,255,.2);border:none;color:#fff;width:34px;height:34px;border-radius:50%;font-size:18px;cursor:pointer;">&times;</button>'
      + '</div>'
      + '<div style="padding:26px 22px 24px;text-align:center;">'
        + '<div style="font-size:13.5px;color:#64748b;font-weight:800;margin-bottom:18px;">입교한 학생 '+students.length+'명 중에서 학생장을 추첨합니다</div>'
        + '<div style="display:flex;align-items:center;justify-content:center;min-height:120px;margin-bottom:20px;padding:24px 14px;background:#fffbeb;border:2px dashed #fcd34d;border-radius:18px;">'
          + '<div id="rlDisplay" class="rl-spinning" style="font-size:42px;font-weight:900;letter-spacing:-1px;line-height:1.1;word-break:keep-all;">준비</div>'
        + '</div>'
        + '<div id="rlBtns" style="display:flex;flex-direction:column;gap:10px;align-items:center;"></div>'
      + '</div>'
    + '</div>';
  document.body.appendChild(modal);
  ...
  function spin(){
    if(ui._rouletteTimer){ clearTimeout(ui._rouletteTimer); ui._rouletteTimer=null; }
    ui._rouletteWinner=null;
    var disp=document.getElementById('rlDisplay');
    var btns=document.getElementById('rlBtns');
    if(!disp||!btns) return;
    disp.className='rl-spinning';
    btns.innerHTML='<div style="color:#94a3b8;font-weight:800;font-size:14px;">🎰 뽑는 중...</div>';
    var elapsed=0, delay=40, duration=2600;
    function step(){
      var idx=Math.floor(Math.random()*names.length);
      disp.textContent=names[idx];
      elapsed+=delay;
      if(elapsed<duration){
        delay=40+Math.floor((elapsed/duration)*230);
        ui._rouletteTimer=setTimeout(step, delay);
      } else {
        ui._rouletteTimer=null;
        var win=students[Math.floor(Math.random()*students.length)];
        ui._rouletteWinner=win;
        disp.textContent='👑 '+win.name;
        disp.className='rl-winner';
        var d=document.getElementById('rlDisplay'); if(d){ d.parentElement.style.animation='rlGlow 1.4s ease infinite'; d.parentElement.style.borderColor='#f59e0b'; d.parentElement.style.background='#fff7ed'; }
        btns.innerHTML=
          '<button onclick="ui.confirmRouletteLeader()" style="width:100%;max-width:340px;background:linear-gradient(135deg,#b45309,#f59e0b);color:#fff;border:none;padding:14px 20px;border-radius:14px;font-weight:900;font-size:16px;cursor:pointer;box-shadow:0 8px 20px rgba(217,119,6,.35);">👑 이 학생을 학생장으로 지정</button>'
          + '<div style="display:flex;gap:10px;width:100%;max-width:340px;">'
            + '<button onclick="ui._rouletteSpin()" style="flex:1;background:#f1f5f9;color:#475569;border:1px solid #e2e8f0;padding:11px 14px;border-radius:12px;font-weight:800;font-size:14px;cursor:pointer;"><i class="fa-solid fa-rotate-right"></i> 다시 돌리기</button>'
