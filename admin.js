...
refs.shuttleReq.on('value', s => {
    ...
    const _os = items.filter(i => i.type === 'osong').length;
    const _tm = items.filter(i => i.type === 'terminal').length;
    const _ai = items.filter(i => i.type === 'airport').length;
    const _carReq = items.filter(i => i.type === 'car').length;
    // [J42] 자차 디폴트 — 차량을 신청하지 않은 '입교완료' 교육생은 자차로 간주(총합 = 입교완료 인원).
    //  입교완료 인원은 courses/{room}/students 수로 산정(신청과 무관하게 확정 인원 기준).
    firebase.database().ref(`courses/${room}/students`).once('value').then(ss => {
        if (state.room !== room) return;
        const completed = Object.values(ss.val()||{}).filter(function(st){ return st && st.name && String(st.name).trim() && st.name!=='undefined'; }).length;   // [J44] 입교완료(유효 이름)만 — 잔여/빈 항목 제외(과대집계 방지)
        const car = Math.max(_carReq, completed - _os - _tm - _ai, 0);
        const total = _os + _tm + _ai + car;
        if (document.getElementById('total-osong')) document.getElementById('total-osong').innerText = _os;
        if (document.getElementById('total-term')) document.getElementById('total-term').innerText = _tm;
        if (document.getElementById('total-air')) document.getElementById('total-air').innerText = _ai;
        ...
        if (totalEl) { totalEl.innerText = total + "명"; totalEl.style.color = "#003366"; }
    }).catch(() => {
        // 폴백(학생 읽기 실패): 기존 방식(신청자만)
        if (document.getElementById('total-osong')) document.getElementById('total-osong').innerText = _os;
        if (document.getElementById('total-term')) document.getElementById('total-term').innerText = _tm;
        if (document.getElementById('total-air')) document.getElementById('total-air').innerText = _ai;
        if (document.getElementById('total-car')) document.getElementById('total-car').innerText = _carReq;
        const totalEl = document.getElementById('dashShuttleTotal');
        if (totalEl) { totalEl.innerText = items.length + "명"; totalEl.style.color = "#003366"; }
    });
});
