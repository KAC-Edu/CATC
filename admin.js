...
renderScreen: function(q) {
        ...
        if(qText) {
            ...
            const _ql = String(q.text || '').length;
            const _qfs = _ql <= 25 ? 46 : _ql <= 45 ? 40 : _ql <= 80 ? 34 : 28;
            qText.style.setProperty('font-size', _qfs + 'px', 'important');
        }
        ...
        if(oDiv) {
            ...
            const _maxLen = Math.max.apply(null, q.options.map(o => String(o).length));
            const _optFs = q.isOX ? 0 : (_maxLen <= 18 ? 34 : _maxLen <= 32 ? 30 : _maxLen <= 50 ? 26 : _maxLen <= 80 ? 22 : 19);
