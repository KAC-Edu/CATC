...
loadAttendanceView: function() {
    ...
    toggleAttendanceMode: function(mode) {
        ...
        if(mode === 'official') {
            ...
            // [J22] OTP 출결 진입 5초 후 자동 전체화면 — 강사가 안내자료+OTP 나란한 화면을 잠시 확인한 뒤 전환.
            //  그 사이 수동으로 전체화면을 열거나, 다른 메뉴/공식QR로 나가면 자동 전환 취소.
            if (this._otpAutoFsTimer) { clearTimeout(this._otpAutoFsTimer); }
            this._otpAutoFsTimer = setTimeout(() => {
                this._otpAutoFsTimer = null;
                try {
    ...
    openOtpFullscreen: function() {
        ...
        ov.innerHTML =
            ...
            + '<button id="otpFsGuideBtn" style="position:fixed; top:22px; left:24px; padding:13px 26px; border:2.5px solid rgba(255,255,255,.55); border-radius:14px; background:rgba(255,255,255,.1); color:#fff; font-size:17px; font-weight:900; cursor:pointer; display:none; align-items:center; gap:9px;"><i class="fa-solid fa-circle-question"></i> 출결 방법 안내</button>'
        ...
        // [J22] '출결 방법 안내' — 안내자료가 있을 때만 버튼 노출, 클릭 시 사진을 위에 겹쳐 전체 표시
        //  (닫으면 이 OTP 전체화면이 그대로 다시 보임 — 전체화면 API를 건드리지 않는 오버레이 방식)
        try {
            const gsrc = (document.getElementById('otpGuideImg')||{}).src;
            const gbtn = document.getElementById('otpFsGuideBtn');
            if (gbtn && gsrc && !/otpGuideImg-missing/.test(gsrc)) {
                const gimg = document.getElementById('otpGuideImg');
                if (gimg && gimg.complete && gimg.naturalWidth > 0) {
                    gbtn.style.display = 'inline-flex';
                    gbtn.onclick = (e)=>{ e.stopPropagation(); ui.openOtpGuideFullscreen(true); };
                }
            }
        } catch(e){}
